...
static constexpr wchar_t kProductName[] = L"FSR Camera";
static constexpr wchar_t kVersion[] = L"0.5.0";
static constexpr wchar_t kUninstallKey[] = L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\FSRCamera";
...
static bool CreateShortcut(const std::wstring& target, const std::wstring& shortcutPath, const std::wstring& workingDir) {
    ...
    link->SetDescription(L"FSR Camera GPU processor and OBS source");
...
static bool RegisterUninstall(const std::wstring& installDir) {
    ...
    const std::wstring app = Join(installDir, L"Fsr3VirtualCamera.exe");
    ...
    SetRegString(key, L"Publisher", L"FSR Camera Project");
...
static std::wstring ShortcutPath() {
    return Join(Join(KnownFolder(FOLDERID_CommonPrograms), L"FSR Camera"), L"FSR Camera.lnk");
}
...
static bool InstallPayload(const std::wstring& installDir, std::wstring& error) {
    ...
    const std::wstring appPath = Join(installDir, L"Fsr3VirtualCamera.exe");
    ...
    if (!WriteResourceToFile(IDR_APP_EXE, appPath)) { error = L"Could not install Fsr3VirtualCamera.exe."; return false; }
...
static int RunUninstall() {
    if (MessageBoxW(nullptr, L"Remove FSR Camera and its OBS plugin?", L"FSR Camera Uninstall",
    ...
    DeleteFileW(Join(installDir, L"Fsr3VirtualCamera.exe").c_str());
    DeleteTree(PluginRoot());
    DeleteFileW(ShortcutPath().c_str());
    const std::wstring shortcutDir = Join(KnownFolder(FOLDERID_CommonPrograms), L"FSR Camera");
    ...
    MessageBoxW(nullptr, L"FSR Camera has been uninstalled.", L"FSR Camera", MB_OK | MB_ICONINFORMATION);
    return 0;
}
...
static std::wstring BrowseForFolder(HWND owner) {
    ...
    dialog->SetTitle(L"Choose where to install FSR Camera");
    std::wstring result;
...
static LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_COMMAND: {
        ...
        if (id == 1003) {
            ...
            SetWindowTextW(gStatus, L"Installing FSR Camera and OBS plugin...");
            UpdateWindow(hwnd);
            std::wstring error;
            const bool ok = InstallPayload(path, error);
            if (ok) {
                SetWindowTextW(gStatus, L"Installed successfully. Restart OBS if it is open.");
                MessageBoxW(hwnd, L"FSR Camera is installed. In OBS, add the 'FSR Camera' source.", kProductName, MB_OK | MB_ICONINFORMATION);
                DestroyWindow(hwnd);
            } else {
                SetWindowTextW(gStatus, L"Installation failed.");
                MessageBoxW(hwnd, error.c_str(), L"FSR Camera Setup", MB_OK | MB_ICONERROR);
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR cmdLine, int show) {
    ...
    HWND hwnd = CreateWindowExW(0, wc.lpszClassName, L"FSR Camera Setup", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU,
    ...
    make(L"STATIC", L"FSR Camera", SS_LEFT, 24, 20, 500, 28, 0);
    make(L"STATIC", L"Installs the GPU camera processor and the native OBS 'FSR Camera' source.", SS_LEFT, 24, 52, 500, 38, 0);
    make(L"STATIC", L"Install location:", SS_LEFT, 24, 98, 180, 20, 0);

    const std::wstring defaultDir = Join(KnownFolder(FOLDERID_ProgramFiles), L"FSR Camera");
