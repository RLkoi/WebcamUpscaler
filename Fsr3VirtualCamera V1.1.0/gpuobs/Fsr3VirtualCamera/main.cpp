...

    {
        ...
        while (gRunning) {
            ...
            if (!initialized || w != lastW || h != lastH) {
                ...
                lastH = h;
                SetStatus(frameGen
                    ? L"Running on GPU. OBS source: FSR Camera. Experimental GPU 2x interpolation enabled."
                    : L"Running on GPU. OBS source: FSR Camera.");
            }
...
static void StartStop() {
    ...
    if (idx < 0) {
        MessageBoxW(nullptr, L"Select an input camera first.", L"FSR Camera Processor", MB_ICONWARNING);
        return;
    }
    if (w > fvc::kMaxWidth || h > fvc::kMaxHeight || w < 64 || h < 64) {
        MessageBoxW(nullptr, L"Resolution must be between 64x64 and 3840x2160.", L"FSR Camera Processor", MB_ICONWARNING);
        return;
    }
    ...
    if (frameGen)
        SetStatus(L"Running. In OBS add Source -> FSR Camera. Experimental 2x interpolation enabled.");
    else
        SetStatus(L"Running. In OBS add Source -> FSR Camera.");
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, LPWSTR, int show) {
    ...
    HWND mainWindow = CreateWindowW(mainClass.lpszClassName, L"FSR Camera Processor",
        WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX,
        CW_USEDEFAULT, CW_USEDEFAULT, 640, 300, nullptr, nullptr, instance, nullptr);
