...
Write-Host ""; Write-Host "WebcamUpscaler V1.0.1 native installer builder (real AMD FSR 3; no WiX)" -ForegroundColor Cyan
...
$sdkRoot = Join-Path $root "FSR3_SDK"
$prepareSdk = Join-Path $root "Prepare-FidelityFX.cmd"
& $prepareSdk
if ($LASTEXITCODE -ne 0) { throw "Could not prepare FidelityFX SDK v1.1.4. Put FidelityFX-SDK-v1.1.4.zip in FSR3_SDK." }
$sdkCache = Join-Path $sdkRoot "_extracted"
$ffxRuntime = Join-Path $sdkCache "PrebuiltSignedDLL\amd_fidelityfx_dx12.dll"
$ffxHeader = Join-Path $sdkCache "ffx-api\include\ffx_api\ffx_upscale.h"
if (-not (Test-Path $ffxRuntime)) { throw "amd_fidelityfx_dx12.dll was not found in the extracted v1.1.4 SDK." }
if (-not (Test-Path $ffxHeader)) { throw "ffx_upscale.h was not found in the extracted v1.1.4 SDK." }

Write-Host ""; Write-Host "[1/5] Building WebcamUpscaler with real AMD FSR 3 Release x64..." -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) { throw "WebcamUpscaler build failed." }
$appExe = Join-Path $root "x64\$Configuration\WebcamUpscaler.exe"
if (-not (Test-Path $appExe)) { throw "WebcamUpscaler.exe was not produced at $appExe" }
Copy-Item $ffxRuntime (Join-Path (Split-Path $appExe -Parent) "amd_fidelityfx_dx12.dll") -Force
...
$spec.displayName = "WebcamUpscaler"
$spec.version = "0.5.0"
$spec.author = "WebcamUpscaler Project"
...
Copy-Item $appExe (Join-Path $payload "WebcamUpscaler.exe") -Force
Copy-Item $ffxRuntime (Join-Path $payload "amd_fidelityfx_dx12.dll") -Force
...
$installer = Join-Path $dist "WebcamUpscaler-V1.0.1-Setup-x64.exe"
...
Write-Host ""
Write-Host "The target PC only needs this one EXE. It contains AMD's signed FidelityFX v1.1.4 DX12 runtime from your SDK ZIP."
